...
function HeroSection() {
  ...
  return (
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-emerald-950">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-emerald-950/70 via-emerald-950/40 to-emerald-950/90 z-[1]" />
      </motion.div>
      ...
      <motion.div className="relative z-20 flex flex-col items-center justify-center text-center h-full max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ opacity }}>
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 0.5, delay: 0.3 }}
          className="w-16 h-16 bg-emerald-500/20 border border-emerald-400/40 flex items-center justify-center mb-8 backdrop-blur-sm">
          <ShieldCheck size={28} weight="fill" className="text-emerald-400" />
        </motion.div>
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
          className="text-emerald-300/80 text-xs sm:text-sm font-semibold uppercase tracking-[0.3em] mb-6" style={{ fontFamily: 'var(--font-sans)' }}>
          {hero.badge}
        </motion.p>
        <div className="overflow-hidden">
          <motion.h1 initial={{ y: '100%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.6, ease: [0.16, 1, 0.3, 1] }}
            ...
            PROTECT YOUR <span className="text-emerald-400 italic font-bold">SPACE</span><br />
            PRESERVE YOUR <span className="text-emerald-400 italic font-bold">HEALTH</span>
          </motion.h1>
        </div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="flex items-center gap-2 mt-6">
          <MapPin size={14} className="text-emerald-400/60" />
          <p className="text-white/30 text-xs uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>Serving Perfect &middot; {business.city} &middot; Greater Harare</p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.4 }} className="flex flex-wrap justify-center gap-4 mt-10">
          <Link to="/contact" className="group inline-flex items-center gap-3 bg-emerald-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-emerald-600 hover:shadow-xl hover:shadow-emerald-500/20 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaPrimary} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
          <Link to="/services" className="group inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:border-emerald-400/50 hover:text-emerald-300 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaSecondary}
          </Link>
        </motion.div>
      </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            ...
            <h2 className="font-heading text-gray-900 leading-[0.95] italic mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              The Perfect<br /><span className="text-emerald-500">Story</span>
            </h2>
            {about.story.map((p, i) => (
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <h2 className="font-heading text-gray-900 leading-[0.95] italic mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Why Choose <span className="text-emerald-500">Perfect</span>
            </h2>
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src={siteData.hero.backgroundImages[0]?.url} alt="Perfect Pest Control Zimbabwe (Pvt) LTD" className="w-full h-[130%] object-cover object-center" loading="lazy" />
